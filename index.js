...
server.on('request', (req, res) => {
    ...
    switch (urlSplits[1]) {
        ...
        case 'planets':
            res.writeHead(200, {'Content-Type': 'application/html'});
            if(urlSplits.length == 3 && urlSplits[2].length > 0) {
                const  myid = urlSplits[2];
                const tmpPlanet = planets.filter(({id}) => id === +myid);
                res.end(JSON.stringify(tmpPlanet));
            } else if (urlSplits.length == 4 && urlSplits[2] === 'color') {
                const mycolor = urlSplits[3];
                const tmpPlanet = planets.filter(({color}) => color === mycolor);
                res.end(JSON.stringify(tmpPlanet));
            }
            else {
